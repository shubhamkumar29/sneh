...
if(!defined('PIEREG_DB_VERSION'))
	...
	define('PIEREG_DB_VERSION','2.0.8');
...
class PieRegister extends PieReg_Base{
...

	{
...

		{
			...
			$errors->add('login-error',apply_filters("piereg_Invalid_username_or_password",__('Invalid email or password.','piereg')));
...

		{
...

			{
				...
				if(strpos(strip_tags($user_login_error),'Invalid email',5) > 6)
...

				{
					...
					$user_login_error = apply_filters('pie_invalid_username_password_msg_txt','<strong>'.ucwords(__("error","piereg")).'</strong>: '.__("Invalid email","piereg").'. <a href="'.$this->pie_lostpassword_url().'" title="'.__("Password Lost and Found","piereg").'">'.__("Lost your password?","piereg").'</a>');
...
////		}
	...
	function ValidPUser(){
		...
		if($_GET['action'] == 'payment_success'){
...

	{
...

		{
			...
			return __('Please','piereg').' <a href="'.pie_login_url().'">'. __('login','piereg').'</a> '.__('to see your profile','piereg');
...

	{
...

		{
...

			{
				...
				$_POST['error'] = apply_filters("piereg_Invalid_Username_or_Password",__("Invalid Email or Password","piereg"));
