...

{
...

	{
		...
		$data .= '<div id="piereg_pie_form_heading " class="pieTitle15">'.$this->data['form']['label'].'</div>';	
		return $data;
...

	{
		...
		return '<input id="username" name="username" class="input_fields" placeholder="'.$this->field['placeholder'].'" type="hidden" value="'.$this->getDefaultValue('username').'" data-errormessage-value-missing="'.$this->field['validation_message'].'"  />';	
		
		/*return '<input id="username" name="username" class="input_fields" placeholder="'.$this->field['placeholder'].'" type="hidden" value="'.$this->getDefaultValue('user').'" data-errormessage-value-missing="'.$this->field['validation_message'].'"  />';	*/
...

	{
		...
		$data .= 'id="'.$this->id.'" name="password" class="'.$this->addClass("input_fields",array("minSize[8]")).'" placeholder="'.$this->field['placeholder'].'" type="password" data-errormessage-value-missing="'.$this->field['validation_message'].'" data-errormessage-range-underflow="'.$this->field['validation_message'].'" data-errormessage-range-overflow="'.$this->field['validation_message'].'" minlength="8" />';
...

	{
		...
		$data .='<input id="'.$this->id.'" name="e_mail" class="'.$this->addClass().'"  '.$this->addValidation().'  placeholder="'.$this->field['placeholder'].'" type="text" value="'.$this->getDefaultValue("e_mail").'" onClick="javascript:set_username();" onBlur="javascript:set_username();" onChange="javascript:set_username();"/>
		<script language="javascript">
		function set_username()
		{
			//alert("hello");
			//alert(document.getElementById("email_1").value);
			var uname = document.getElementById("email_1").value;
...

	{
...

		{
...

			{
				...
				if($this->field['value'][$a] == 'a_email' || $this->field['value'][$a] == 'ts')
				{
					
					$data .= '<div class="radio_one">';
...
*/
...

	{
...

		{
			...
			$data .= '<input name="pie_submit" class="blueButton" type="submit" value="'.$this->field['text'].'" />';	
...

		{
			...
			$data .= '<input name="pie_reset" class="blueButton" type="reset" value="'.$this->field['reset_text'].'" />';		
...

	{
...

		{
			...
			$errors->add( $slug , '<strong>'.__(ucwords('Error'),'piereg').'</strong>: '.apply_filters("piereg_Invalid_Username",__(' Invalid Email','piereg' )));
...

		{
			...
			$errors->add( $slug , '<strong>'.__(ucwords('Error'),'piereg').'</strong>: '.apply_filters("piereg_Username_already_exists",__(' Email already exists','piereg' )));
