...
if ( get_option( 'woocommerce_ship_to_billing_address_only' ) === 'no' && get_option( 'woocommerce_calc_shipping' ) !== 'no' ) {
	...
	$get_addresses    = apply_filters( 'woocommerce_my_account_get_addresses', array(
		...
		'shipping' => __( 'Service Address', 'woocommerce' )
	), $customer_id );
} else {
...
<div class="fontSize18 line20 paddingTB15"><?php echo $page_title; ?></div>
...
<?php foreach ( $get_addresses as $name => $title ) : ?>
	...
	<div class="col-<?php echo ( ( $col = $col * -1 ) < 0 ) ? 1 : 2; ?> address">
		<header class="title">
			...
			<a href="<?php echo wc_get_endpoint_url( 'edit-address', $name ); ?>" class="edit button smallBtn" style="margin-top:20px"><?php _e( 'Edit', 'woocommerce' ); ?></a>
		</header>
		<address>
			<?php
